...
namespace DripOldDriver.Cache
{
	/// <summary>DripOldDriver Asp.net 缓存操作类</summary>
	public class WebCache : ICache
	{
        ...
        /// <typeparam name="T">数据类型</typeparam>
        /// <param name="key">要获取的缓存键</param>
        /// <param name="acquire">设置缓存的委托</param>
        /// <param name="absExpire">是否绝对过期</param>
        /// <param name="expire">缓存的时间</param>
        /// <returns>缓存的数据</returns>
        public T Get<T>(string key, Func<T> acquire, bool absExpire = false, int expire = 0)
        {
            var value = Get<T>(key);
            ...
            {
                var result = acquire();
                Add(key, result, absExpire, expire);
                return result;
            }
        }
        #endregion
